...
Y=A*N;
Y=C_H+C_H_starr+G;
C=((1-ALPHA)^(1/ETA)*C_H^((ETA-1)/ETA)+ALPHA^(1/ETA)*C_F^((ETA-1)/ETA))^(ETA/(ETA-1));

Y_starr=A_starr*N_starr;
Y_starr=C_F+C_F_starr+G_starr;
C_starr=(ALPHA_starr^(1/ETA)*C_H_starr^((ETA-1)/ETA)+(1-ALPHA_starr)^(1/ETA)*C_F_starr^((ETA-1)/ETA))^(ETA/(ETA-1));
...
ramsey_model(instruments=(C_H,C_F,C_H_starr,C_F_starr,N,G,N_starr,G_starr),planner_discount=BETA);
