function x = Model_draft_4_testplan5_steadystate_helper(x0,PHI,SIGMA,GAMMA,CHI,ALPHA,ALPHA_starr,h)
    ...
    % else
        ...
        x = fsolve(@(x) x-h*((x/h)^(-PHI/SIGMA)+(1/CHI)^(-1/GAMMA)*(x/h)^(-PHI/GAMMA)), x0,options);
        %res=fsolve(@(x) [ ...
            ...
            %x0,options);
        %res=fsolve(@(x) [ ...
            %x(1)-h*(x(1)^(-PHI/SIGMA)+(1/CHI)^(-1/GAMMA)*x(1)^(-PHI/GAMMA)); ...
            %x(2)-(1-h)*(x(1)^(-PHI/SIGMA)+(1/CHI)^(-1/GAMMA)*x(1)^(-PHI/GAMMA))], ...
            %x0,options);
        %x =  [res(1),res(2)];
    %end
end
